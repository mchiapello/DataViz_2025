...
```r
# Pie chart - conta classi in mpg
ggplot(mpg, aes(x = "", fill = class)) +
  geom_bar(width = 1) +
  coord_polar(theta = "y") +
  theme_void()

# Con valori pre-calcolati
library(dplyr)
class_counts <- mpg %>% 
  count(class)

ggplot(class_counts, aes(x = "", y = n, 
                         fill = class)) +
  geom_col(width = 1) +
  coord_polar(theta = "y") +
  theme_void()
...
::: {.callout-tip}
## Nota tecnica
Pie chart = bar chart stacked + coord_polar()

- `geom_bar(width = 1)` crea barra singola
- `coord_polar(theta = "y")` mappa altezza → angolo
- `theme_void()` rimuove assi
:::
